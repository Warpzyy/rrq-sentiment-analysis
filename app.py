...
def load_assets():
    try:
        ...
        # 2. Bangun Rangka Model (Disesuaikan dengan hasil error log tadi)
        model = tf.keras.Sequential([
            # Input dim 5000, output dim 32, input length 50
            tf.keras.layers.Embedding(input_dim=5000, output_dim=32, input_length=50),
            # Kita ganti ke 32 units sesuai "Received: 128" (128/4=32)
            tf.keras.layers.LSTM(32), 
            # Kita sesuaikan Dense layer-nya juga
            tf.keras.layers.Dense(16, activation='relu'), 
            tf.keras.layers.Dense(3, activation='softmax')
        ])
        
        # Build model agar siap menerima bobot
        model.build(input_shape=(None, 50)) 
        
        # 3. Masukkan bobotnya
        # Pastikan file model_weights.weights.h5 sudah di-upload ulang jika ada perubahan
